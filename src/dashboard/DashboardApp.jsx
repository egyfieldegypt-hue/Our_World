  ...
  return (
    <div className="flex max-w-3xl flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Btn onClick={reset}>{t('dashboard.texts.reset')}</Btn>
        {saved && <span className="text-sm text-gold">✓ {t('dashboard.actions.saved')}</span>}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-ink text-cream">
      <header className="sticky top-0 z-40 border-b border-white/8 bg-ink/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <div className="flex min-w-0 items-center gap-3">
            <img
              ...
              alt="بينّا"
              draggable={false}
              className="h-11 w-auto shrink-0 drop-shadow-[0_2px_10px_rgba(196,162,103,0.25)]"
            />
            <div className="min-w-0">
              <h1 className="truncate font-display text-lg font-black">{t('dashboard.title')}</h1>
          ...
          <div className="flex shrink-0 items-center gap-2.5">
            ...
            <button
              ...
              className="rounded-full border border-white/10 px-4 py-2 text-xs font-bold text-cream/60 transition-colors hover:border-rose/50 hover:text-rose"
            >
              {t('dashboard.signOut')}
            </button>
          </div>
        ...
        <nav className="mx-auto max-w-6xl overflow-x-auto px-4 pb-3" aria-label={t('dashboard.title')}>
